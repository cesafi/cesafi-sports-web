import { format as formatDate, formatDistanceToNow, isToday, isYesterday, isThisWeek, isThisYear, Locale } from 'date-fns';

/**
 * Date utility functions for consistent date formatting across the dashboard
 */
...
export function formatSmartDate(
  ...
  if (isThisWeek(dateObj)) {
    return showTime 
      ? `${formatDate(dateObj, 'EEEE')} at ${formatTime(dateObj, options.timeFormat)}`
      : formatDate(dateObj, 'EEEE');
  }
  
  if (isThisYear(dateObj)) {
    return showTime 
      ? `${formatDate(dateObj, 'MMM d')} at ${formatTime(dateObj, options.timeFormat)}`
      : formatDate(dateObj, 'MMM d');
  }
  
  // Older dates
  const yearFormat = options.showYear !== false ? 'MMM d, yyyy' : 'MMM d';
  const dateStr = formatDate(dateObj, yearFormat);
...
export function formatDetailedDate(date: Date | string | number | null | undefined): string {
  ...
  return formatDate(dateObj, 'PPP \'at\' p');
}
