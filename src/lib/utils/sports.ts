...
export const getSportIcon = (sportName: string): string => {
  ...
  if (normalizedSportName.includes('football') || normalizedSportName.includes('soccer'))
    return 'Football';
  if (normalizedSportName.includes('tennis')) return 'Tennis';
  if (normalizedSportName.includes('badminton')) return 'Badminton';
  if (normalizedSportName.includes('table tennis') || normalizedSportName.includes('ping pong'))
    return 'TableTennis';
  if (normalizedSportName.includes('baseball')) return 'Baseball';
  if (normalizedSportName.includes('softball')) return 'Softball';
  if (normalizedSportName.includes('track') || normalizedSportName.includes('field'))
    return 'Activity';
  if (normalizedSportName.includes('running') || normalizedSportName.includes('sprint'))
    return 'Zap';
  if (normalizedSportName.includes('marathon')) return 'MapPin';
  if (normalizedSportName.includes('relay')) return 'Users';
  if (normalizedSportName.includes('swimming') || normalizedSportName.includes('diving'))
    return 'Waves';
  if (
    normalizedSportName.includes('boxing') ||
    normalizedSportName.includes('taekwondo') ||
    normalizedSportName.includes('karate') ||
    normalizedSportName.includes('judo')
  )
    return 'Fist';
  ...
  if (normalizedSportName.includes('cheer') || normalizedSportName.includes('dance'))
    return 'Sparkles';
  if (normalizedSportName.includes('sepak takraw')) return 'Circle';
  if (normalizedSportName.includes('archery')) return 'Target';
  if (normalizedSportName.includes('weight') || normalizedSportName.includes('lifting'))
    return 'Dumbbell';
