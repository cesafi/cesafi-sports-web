...
export const scheduleKeys = {
  ...
  infinite: (options: SchedulePaginationOptions) => [...scheduleKeys.all, 'infinite', options] as const,
...
export function useInfiniteSchedule(
  ...
  return useInfiniteQuery({
    ...
    select: (data) => ({
      ...
      matches: data.pages.flatMap(page => 
        page.success && page.data ? page.data.matches : []
      ),
      hasNextPage: data.pages[data.pages.length - 1]?.success 
        ? data.pages[data.pages.length - 1].data?.hasMore ?? false 
        : false,
      hasPreviousPage: data.pages[0]?.success 
        ? data.pages[0].data?.hasMore ?? false 
        : false,
      totalCount: data.pages[0]?.success 
        ? data.pages[0].data?.totalCount ?? 0 
        : 0,
    }),
...
export function useScheduleByDate(filters: ScheduleFilters = {}) {
  ...
  return useQuery({
    ...
    select: (data) => {
      ...
      // Group matches by date and sort date keys
      const groupedMatches = groupMatchesByDate(data.data);
